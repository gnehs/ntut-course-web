...
import { Input } from '../components/ui-kit/Input.jsx'
import { Loader } from '../components/ui-kit/Loader.jsx'
import { fetchCourse, fetchMicroPrograms } from '../lib/courseApi.js'
    ...
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {filteredPrograms.map((program) => (
          <Card className="cursor-pointer p-3 transition-transform duration-200 hover:-translate-y-1 hover:shadow-[0_10px_20px_0_rgba(0,0,0,var(--vs-shadow-opacity,0.05))] active:translate-y-[5px] active:shadow-none" key={program.id} to={`/mprogram/${dataset.year}/${dataset.sem}/${program.id}`}>
...
export function MProgramDetailPage() {
  ...
  useEffect(() => {
    ...
    setError(null)
    async function load() {
      const [programList, allCourses] = await Promise.all([fetchMicroPrograms(year, sem), fetchCourse(year, sem)])
      const foundProgram = programList.find((item) => item.id === id)
      const courseIds = foundProgram?.course || []
      const result = foundProgram ? allCourses.filter((course) => courseIds.includes(course.id)) : []
      if (!cancelled) {
  ...
  function addProgramCourses() {
    ...
    if (previous && previous !== programName && !confirm(`你先前已將「${previous}」之課程加入我的課程，此行為會導致課程過多，要繼續嗎？`)) {
      return
    }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div><h1>{programName}</h1></div>
        <div className="flex flex-wrap justify-end">
          <Button active={showConflictCourse} onClick={() => setShowConflictCourse((value) => !value)}>
            <i className={`bx ${showConflictCourse ? 'bx-check' : 'bx-x'}`} />衝堂課程
          </Button>
          {!isInMyCourse ? (
            <Button primary onClick={addProgramCourses}><i className="bx bx-plus" />加入我的課程</Button>
          ) : (
            <Button danger onClick={removeProgramCourses}><i className="bx bx-minus" />從我的課程移除</Button>
          )}
        </div>
      </div>
      {error ? <Alert danger><strong>擷取資料時發生了錯誤</strong><pre>{String(error.message || error || 'Error')}</pre></Alert> : null}
      {courses.length ? <CourseList courses={courses} showTimetable showConflictCourse={showConflictCourse} year={year} sem={sem} /> : null}
      {!courses.length && program ? (
        <Alert>
          ...
          {program.href ? (
            <>
              <br />
              <a href={`https://aps.ntut.edu.tw/course/tw/${program.href}`} target="_blank" rel="noreferrer">前往原始網頁</a>
              看看原本的資料
            </>
          ) : null}
        </Alert>
