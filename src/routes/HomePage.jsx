...
import { displayDepartment } from '../lib/courseUtils.js'
  ...
  return (
    <PageSection title="接下來的課程">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((course) => (
          <Card className="px-4 py-3" key={course.id} to={`/course/${dataset.year}/${dataset.sem}/${course.id}`}>
            <CardTitle>{course.name.zh}</CardTitle>
  ...
  return (
    <div ref={pageRef} className="grid gap-6">
      ...
      <PageSection title="課程">
        <HomeCardGrid items={[
          { to: `/advanced-search?year=${dataset.year}&sem=${dataset.sem}&d=${dataset.department}`, title: '進階搜尋', text: '依條件搜尋課程', icon: 'bx bx-search' },
          { to: '/class', title: '班級課表', text: '查看各班上課時間表', icon: 'bx bx-time' },
          { to: '/mprogram', title: '微學程', text: '查詢微學程課程', icon: 'bx bx-book-content' },
          { to: '/my-course', title: '我的課程', text: '查看已儲存的課程', icon: 'bx bx-user' },
        ]} />
      </PageSection>
      <PageSection title="實用">
        <HomeCardGrid items={[
          { to: standardURL, title: '課程標準', text: '查看各系所畢業標準等相關資訊', icon: 'bx bxs-graduation' },
          { to: '/emptyroom', title: '尋找空教室', text: '查看沒有課程進行的教室', icon: 'bx bx-ghost' },
          { to: '/withdrawal', title: '退選率', text: '查看所有教師的退選率', icon: 'bx bx-user-x' },
          { to: '/calendar', title: '行事曆', text: '查看學校行事曆', icon: 'bx bx-calendar' },
          { to: `/widget?year=${dataset.year}`, title: 'iOS 小工具', text: '在桌面上檢視接下來的課程', icon: 'bx bx-extension' },
          { to: `/add-calendar?year=${dataset.year}`, title: '新增課程到行事曆', text: '將我的課程匯入至行事曆', icon: 'bx bx-calendar-plus' },
        ]} />
      </PageSection>
      <PageSection title="其他">
        <HomeCardGrid items={[
          { to: '/doc', title: '文件', text: 'API 文件與嵌入頁面相關功能介紹與說明', icon: 'bx bx-file' },
          { to: '/changelog', title: '更新日誌', text: '查看本站最近的更新日誌', icon: 'bx bx-history' },
          { to: '/about', title: '關於', text: '關於本網站', icon: 'bx bx-info-circle' },
          { to: '/privacy', title: '隱私權政策', text: '隱私權政策', icon: 'bx bx-info-circle' },
          { to: '/status', title: '擷取狀態', text: '查看爬蟲資料擷取狀態', icon: 'bx bx-terminal' },
          { to: '/settings', title: '設定', text: '課程資料庫、資料匯出等', icon: 'bx bx-cog' },
        ]} />
      </PageSection>
      <PageSection title="贊助商廣告">
      <AdsByGoogle />
      <p style={{ textAlign: 'center', fontSize: '.75em', opacity: .75 }}>
        本站資料擷取自 <a href="https://aps.ntut.edu.tw/course/tw/course.jsp" target="_blank" rel="noreferrer">國立臺北科技大學課程系統</a>，資料僅供參考，可能會有所遺漏或錯誤，正式資料仍以學校公佈為主。
      </p>
      </PageSection>
    </div>
  )
