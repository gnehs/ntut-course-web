	...
	const result = useMemo(() => {
		...
		try {
			...
			if (searchVal) {
				data = data.filter(
					(course) =>
						regexMatch(course.name?.zh, searchVal) ||
						((course.class || []).some((item) => /體育/.test(item.name)) &&
							regexMatch(course.notes, searchVal)),
				);
			}
			if (searchCourseId)
				data = data.filter((course) => String(course.id || '').startsWith(searchCourseId));
			if (searchTeacher)
				data = data.filter((course) =>
					regexMatch((course.teacher || []).map((item) => item.name).join(''), searchTeacher),
				);
			if (searchClass)
				data = data.filter((course) =>
					regexMatch((course.class || []).map((item) => item.name).join(''), searchClass),
				);
			...
			if (categoryFilter.length) {
				data = data.filter(
					(course) =>
						(course.class || []).some((item) => /博雅/.test(item.name)) &&
		...
		setTimetableFilter((current) => {
			...
			if (date && slot) {
				...
				for (const key of Object.keys(next))
					next[key] = hasSlot ? next[key].filter((item) => item !== slot) : [...next[key], slot];
			} else {
	...
	return (
		<div className='grid gap-4'>
			...
			<Dialog
				...
				footer={
					<div className='flex w-full justify-end gap-2'>
						<Button className='m-0' onClick={resetCategory}>
							重置
						</Button>
						<Button primary className='m-0' onClick={() => setCategoryDialog(false)}>
							完成
						</Button>
					</div>
				}
					...
					{Object.entries(categoryFilterList).map(([key, value]) => (
						...
						>
							<Checkbox
								...
								onCheckedChange={() => setCategoryFilter((items) => toggleArrayValue(items, value))}
							/>
							{key}
						</label>
					))}
			...
			<Dialog
				...
				footer={
					<div className='flex w-full justify-end gap-2'>
						<Button className='m-0' onClick={resetStandardDialog}>
							重置
						</Button>
						<Button primary className='m-0' onClick={() => setCourseStandardDialog(false)}>
							完成
						</Button>
					</div>
				}
			...
			<Dialog
				...
				footer={
					<div className='flex w-full justify-end gap-2'>
						<Button className='m-0' onClick={resetTimetable}>
							重置
						</Button>
						<Button primary className='m-0' onClick={() => setTimetableDialog(false)}>
							完成
						</Button>
					</div>
				}
			>
				<p className='m-0 text-sm opacity-75'>
					點擊課表上的星期或節次可以一次選取整個行或列，點左上角可一次選取整張表。
				</p>
