...
export function MyCoursePage() {
  ...
  const credit = useMemo(() => (courses || []).reduce((sum, course) => sum + Number(course.credit || 0), 0), [courses])
  const hours = useMemo(() => (courses || []).reduce((sum, course) => sum + Number(course.hours || 0), 0), [courses])
  ...
  return (
    <div>
      ...
      <p style={{ marginTop: '-1em' }}>你可以在這裡儲存一些課程供未來選課時參考用，在此處的課程會與其他課程比對並顯示是否衝堂。</p>
      {!courses.length ? (
        <Alert>
          <strong>尚未儲存任何課程</strong><br />
      ...
      {message ? <Alert><strong>匯入完成！</strong><br />{message}</Alert> : null}
      <h3>贊助商廣告</h3>
      <AdsByGoogle />
    </div>
  )
