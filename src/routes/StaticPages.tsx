...
export function AboutPage() {
  return (
    <div>
      <h1>關於</h1>
      <p>
        本網站與爬蟲由 <a href="https://gnehs.net" target="_blank" rel="noreferrer">勝勝</a> 開發。
      </p>
      <h2>功能</h2>
      <ul className="list-disc space-y-1 pl-6">
        ...
        <li>自動偵測衝堂課程</li>
        <li>記錄課程供選課參考</li>
        <li>提供 <Link to="/doc">API</Link> 供公眾使用</li>
      </ul>
      <h2>技術細節</h2>
      <p>本站資料擷取自 <a href="https://aps.ntut.edu.tw/course/tw/course.jsp" target="_blank" rel="noreferrer">國立臺北科技大學課程系統</a>，資料僅供參考，可能會有所遺漏或錯誤，正式資料仍以學校公佈為主。若資料有問題或有任何建議，歡迎於 GitHub 提出 Issue 或發送 Pull Request。</p>
      <h2>聯絡開發者</h2>
      <p>若對北科好朋友有任何建議或找到 Bug，請聯繫 ntut-course-web[at]gnehs.net（請將 [at] 替換為 @），或在 <a href="https://github.com/gnehs/ntut-course-web" target="_blank" rel="noreferrer">GitHub</a> 上發 issue。</p>
      <h2>Special Thanks</h2>
      <ul className="list-disc space-y-1 pl-6">
        <li><a href="https://boxicons.com/" target="_blank" rel="noreferrer">Boxicons</a></li>
        ...
        <li><a href="https://tanstack.com/router/latest" target="_blank" rel="noreferrer">TanStack Router</a></li>
      </ul>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      ...
      <ul className="list-disc space-y-1 pl-6">
        ...
        <li>在任一網址後方加入 <code>?mode=iframe</code> 即可使用嵌入功能。</li>
      </ul>
      <h1>API 文件</h1>
      <h2>注意</h2>
      <ul className="list-disc space-y-1 pl-6">
      ...
      {[
        ...
        <div key={item} className="rounded-[8px] border border-[rgba(var(--vs-text),0.1)] bg-[rgb(var(--vs-background))] px-4 py-3">
...
export function PrivacyPage() {
  return (
    <div className="space-y-4">
      <h1>隱私權政策</h1>
      <p>上次更新：2022/6/15</p>
      <p>非常歡迎您光臨「北科課程好朋友」（以下簡稱本網站），為了讓您能夠安心使用本網站的各項服務與資訊，特此向您說明本網站的隱私權保護政策，以保障您的權益，請您詳閱下列內容：</p>
      ...
      <h3>八、聯繫管道</h3>
      <p>對於本站之隱私權政策有任何疑問，或者想提出變更、移除個人資料之請求，請聯繫 ntut-course-web[at]gnehs.net（請將 [at] 替換為 @）。</p>
    </div>
  )
}
...
export function ChangelogPage() {
  ...
  const items: [string, ChangeItem[]][] = [
    ...
    ['2023-02-21', ['新增了重設資料庫選項，可以在此重設異常的課程資料庫。', '修正了進入課程頁面再返回進階搜尋頁面時，無法載入課程資料的錯誤。']],
    ['2023-01-13', ['新增了班級頁面的預覽功能。', '修正了班級頁面在瀏覽過往班級時，會顯示錯誤建議的錯誤。', '修改了班級連結格式。']],
    ['2023-01-12', ['新增了預覽功能，發送在 SNS 上的課程與教師的網址會自動顯示預覽。', '新增 Sitemap 產生器']],
    ['2022-12-30', ['新增了「行事曆」頁面']],
    ['2022-12-29', ['更新了「新增到行事曆」的自動填寫功能，現在能根據學校行事曆自動填上起始與結束日', '新增了「行事曆」API']],
    ...
    ['2022-11-23', [
      '重新設計了課程頁面卡片區域',
      ['退選率', ['新增退選率 API', '除了日間部外，也一起統計了進修部與研究所等課程，提供更加精確的退選率', '課程頁面顯示退選率與其介紹', '退選率頁面改由 API 提供服務，加快了讀取速度！']],
    ]],
    ['2022-10-05', ['尋找空教室支援了星期選擇功能']],
    ['2022-10-03', ['現在課程資料會自動儲存在瀏覽器中了！除了能夠帶來高速的讀取速度，重新整理時也無需下載新資料了', '修正了重複點擊課程標準中相同科系時，造成的錯誤', '修正了一些錯字']],
    ['2022-09-28', [['重新設計的課表', ['顯示更多有用資訊', '自動合併連堂課程', '課表將依據課程進行動態調整']]]],
    ['2022-09-27', ['「退選率」現在預設會抓取十個學期的資料']],
    ['2022-09-19', ['新增了「新增到行事曆」自訂區間選項', '於「課程頁面」瀏覽器標題中新增了課號', '修正了接下來的課程排序']],
    ...
    <div>
      ...
      {items.map(([date, changes]) => (
        <section key={date}>
          <h2>{date}</h2>
          <ul>
            {changes.map((item) => Array.isArray(item)
              ? <li key={item[0]}>{item[0]}<ul>{item[1].map((child) => <li key={child}>{child}</li>)}</ul></li>
              : <li key={item}>{item}</li>
            )}
          </ul>
        </section>
...
export function NotFoundPage() {
  return <div><h1>找不到頁面</h1><p>你要找的頁面不存在。</p><Button as={Link} to="/">回首頁</Button></div>
}
    ...
    <div>
      ...
      <div className="space-y-2">
        {runs.map((run) => (
          ...
          >
            <div className={`grid h-8 w-8 place-items-center rounded-full text-lg ${run.status === 'completed' ? 'bg-[rgba(70,201,58,0.2)] text-[rgb(70,201,58)]' : 'bg-[rgba(var(--vs-primary),0.2)] text-[rgb(var(--vs-primary))]'}`}>
              {run.status === 'in_progress' ? <i className="bx bx-loader bx-spin" /> : run.status === 'completed' ? <i className="bx bx-check" /> : <i className="bx bx-question-mark" />}
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-bold">{parseName(run.name)}</div>
              <div className="text-sm opacity-75">
                {run.event === 'schedule' ? <i className="bx bx-calendar" /> : run.event === 'push' ? <i className="bx bx-git-commit" /> : run.event === 'dynamic' ? <i className="bx bx-revision" /> : <i className="bx bx-question-mark" />}
                <span> • </span>
