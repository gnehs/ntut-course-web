...
export function TeacherPage() {
  ...
  const [rates, setRates] = useState<{ all: WithdrawalRateMap; r3: WithdrawalRateMap; r5: WithdrawalRateMap } | null>(null)
  const [error, setError] = useState<unknown>(null)
  useEffect(() => {
    let cancelled = false
    Promise.all([fetchWithdrawalRate(''), fetchWithdrawalRate('-recent-3-years'), fetchWithdrawalRate('-recent-5-years'), fetchWithdrawal('')]).then(([all, r3, r5, data]) => {
      if (cancelled) return
      if (!all[id]) {
        setError('找不到教師')
        setTeacher(false)
  ...
  return (
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <Card><CardTitle>{teacher.withdraw}</CardTitle><p>退選人數</p></Card>
        <Card><CardTitle>{teacher.people}</CardTitle><p>選課人數</p></Card>
        <Card><CardTitle>{teacher.course?.length || 0}</CardTitle><p>課程數</p></Card>
        {stats.map((item) => <Card key={item.name}><CardTitle>{item.value}%</CardTitle><p>{item.name}</p></Card>)}
      </div>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div><h3>課程</h3></div>
        <div className="flex flex-wrap justify-end gap-1">
          <Button className="m-0" active={department === 'main'} onClick={() => selectDepartment('main')}>日間部</Button>
          <Button className="m-0" active={department === '進修部'} onClick={() => selectDepartment('進修部')}>進修部</Button>
          <Button className="m-0" active={department === '研究所(日間部、進修部、週末碩士班)'} onClick={() => selectDepartment('研究所(日間部、進修部、週末碩士班)')}>研究所</Button>
        </div>
      </div>
        ...
        {courses.map((course: TeacherWithdrawalCourse) => (
          ...
          >
            <div>
              ...
              <div className="text-xs opacity-80">{course.year} 年{course.sem === '1' ? '上' : '下'}學期</div>
            </div>
            <div>
              <div className="text-base font-bold">{course.courseType} {course.name?.zh}</div>
