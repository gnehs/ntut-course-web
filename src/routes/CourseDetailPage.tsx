  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <Card><CardTitle>{currentCourse.id}</CardTitle><p>課號</p></Card>
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-3">
        <InfoCard icon="bx-info-circle" title="課程資訊" items={[
          ['課程標準', `${course.courseType || ''} ${course.courseType ? courseStandard[course.courseType] || '' : ''}`],
...
function InlineLinks({ items, fallback = '' }: { items: { to: string; label: string }[]; fallback?: React.ReactNode }) {
...
function HtmlText({ text, as: Component = 'span' }: { text: string; as?: keyof React.JSX.IntrinsicElements }) {
...
function timeSince(date: Date) {
  ...
  const intervals: [string, number][] = [[' 年', 31536000], [' 月', 2592000], [' 天', 86400], [' 小時', 3600], [' 分鐘', 60]]
