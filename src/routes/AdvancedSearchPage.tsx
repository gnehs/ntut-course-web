...
export function AdvancedSearchPage() {
  ...
  const restoredQuery = useMemo<AdvancedSearchQuery>(() => safeParseJson(params.get('q'), {}), [location.search])
  ...
  const [academyFilter, setAcademyFilter] = useState<string[]>(restoredQuery.af ? String(restoredQuery.af).split(',') : [])
  const [timetableFilter, setTimetableFilter] = useState<Record<string, string[]>>(restoredQuery.tf || structuredClone(emptyTimetableFilter))
  ...
  const academyList = useMemo<string[]>(() => [...new Set<string>((departmentData || []).map((item) => item.category))], [departmentData])
  ...
  const searchResult = useMemo(() => {
    ...
    try {
      ...
      if (academyFilter.length && departmentData) {
        ...
        const filterOutClass = departmentData.filter((item) => filterOutAcademy.includes(item.category)).flatMap((item) => item.class || []).map((item) => item.name)
...
function AdvancedSearchSidebarContent({
  ...
  return (
    <div className="grid gap-3">
      ...
      <SearchSection title="依時間篩選" open={(Object.values(timetableFilter) as string[][]).some((items) => items.length)}>
...
function SearchSection({ title, open = false, children }: { title: string; open?: boolean; children: React.ReactNode }) {
