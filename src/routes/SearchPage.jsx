...
import { Field } from '../components/ui-kit/Field.jsx'
...
export function SearchPage() {
  ...
  const initialState = useMemo(() => ({
    ...
    category: safeParseJson(query.get('category'), []),
  ...
  const [timetableFilter, setTimetableFilter] = useState(initialState.timetable)
  ...
  useEffect(() => {
    const next = { year, sem, d: department }
  ...
  function toggleLesson(date, slot) {
    setTimetableFilter((current) => {
      ...
      if (date && slot) {
        ...
        const any = Object.values(next).some((items) => items.includes(slot))
        for (const key of Object.keys(next)) next[key] = any ? next[key].filter((item) => item !== slot) : [...next[key], slot]
      } else {
        const any = Object.values(next).some((items) => items.length)
        for (const key of Object.keys(next)) next[key] = any ? [] : timetable.slice(0, -1)
      }
      return next
    })
  }
