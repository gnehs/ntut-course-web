...
export function EmptyRoomPage() {
  ...
  const { categoryList, roomList } = useMemo(() => {
    const result: { categoryList: string[]; roomList: EmptyRoom[] } = { categoryList: [], roomList: [] }
  ...
  return (
    <div>
      {onError ? <Alert danger><strong>擷取資料時發生了錯誤</strong><br /><pre>{errorMessage(onError)}</pre></Alert> : null}
