...
import { buildCourseCalendar, trimEllip } from '../lib/courseUtils.js'
...
function resolveStartDate(calendar, rocYear, sem) {
  const startDays = calendar.filter((item) => item.summary?.includes('開學')).map((item) => item.start?.date || item.start)
  const target = semesterDate(rocYear, sem, 'start')
  return startDays.find((item) => new Date(item) >= target) || isoDate(target)
}

function resolveEndDate(calendar, rocYear, sem) {
  const endDays = calendar.filter((item) => item.summary === '期末考試').map((item) => item.end?.date || item.end)
  const target = semesterDate(rocYear, sem, 'end')
  return endDays.find((item) => new Date(item) >= target) || isoDate(target)
}

function semesterDate(rocYear, sem, type) {
