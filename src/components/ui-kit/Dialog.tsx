...
export function Dialog({ open, title, children, footer, onClose }: DialogProps) {
	return (
		<ShadcnDialog open={open} onOpenChange={(value) => !value && onClose?.()}>
			<DialogContent className='rounded-overlay w-[min(420px,calc(100vw-2rem))] gap-4 border border-[rgba(var(--vs-text),0.1)] bg-[rgb(var(--vs-background))] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.24)]'>
