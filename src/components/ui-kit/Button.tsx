...
type ButtonProps = {
  ...
  primary?: boolean
  type?: string
} & Omit<React.ComponentPropsWithoutRef<'button'>, 'type'> & Record<string, unknown>

export function Button({ as: Component = 'button', className = '', active = false, icon = false, danger = false, primary = false, type = 'button', ...props }: ButtonProps) {
  const classes = cn(
