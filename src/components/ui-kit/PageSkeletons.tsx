...
function SkeletonBlock({ className = '' }: { className?: string }) {
  return <div className={`animate-pulse rounded-[8px] bg-[rgba(var(--vs-text),0.1)] ${className}`} />
}

function SkeletonCard({ children, className = '' }: { children?: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded-[8px] bg-[rgb(var(--vs-background))] p-4 shadow-[0_5px_20px_0_rgba(0,0,0,var(--vs-shadow-opacity,0.05))] ${className}`}>
      {children}
    </div>
  )
}
...
function StatCardsSkeleton({ count = 3, className = 'grid-cols-3' }: { count?: number; className?: string }) {
  return (
  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-[rgba(var(--vs-text),0.1)] bg-[rgb(var(--vs-background))]">
      {Array.from({ length: rows }, (_, index) => (
        <div key={index} className={`grid gap-2 px-4 ${compact ? 'py-2' : 'py-3'} ${index > 0 ? 'border-t border-[rgba(var(--vs-text),0.1)]' : ''}`}>
...
export function CardGridSkeleton({ count = 10, columns = 'grid-cols-3 lg:grid-cols-5' }: { count?: number; columns?: string }) {
  return (
...
export function ClassIndexSkeleton() {
  return (
    <div>
      ...
      <div className="mt-3"><CardGridSkeleton count={10} /></div>
    </div>
  )
}
    ...
    <div>
      ...
      {Array.from({ length: 4 }, (_, index) => (
        <section key={index} className="mt-5">
          ...
          {index === 1 ? (
            codeBlock ? <SkeletonBlock className="mt-3 h-[360px] w-full" /> : <ListRowsSkeleton rows={4} compact />
          ) : null}
        </section>
      ))}
    </div>
    ...
    <div>
      ...
      <div className="mt-4 overflow-hidden rounded-[4px] border border-[rgba(var(--vs-text),0.1)]">
        {Array.from({ length: 8 }, (_, index) => (
          <div key={index} className={`flex items-center gap-2 bg-[rgb(var(--vs-background))] p-2.5 ${index > 0 ? 'border-t border-[rgba(var(--vs-text),0.1)]' : ''}`}>
...
export function TeacherSkeleton() {
  return (
    <div>
      <SkeletonHeading />
      <div className="mt-3"><StatCardsSkeleton count={6} className="sm:grid-cols-3 lg:grid-cols-6" /></div>
      <div className="mt-6 flex items-center justify-between gap-3">
        <SkeletonBlock className="h-6 w-20" />
        <div className="flex gap-1"><SkeletonBlock className="h-10 w-20" /><SkeletonBlock className="h-10 w-20" /><SkeletonBlock className="h-10 w-20" /></div>
      </div>
      <div className="mt-2 grid gap-3 md:grid-cols-2">
        {Array.from({ length: 6 }, (_, index) => <SkeletonCard key={index} className="h-20" />)}
      </div>
    </div>
  )
}
...
export function StandardPickerSkeleton() {
  return (
    <div>
      ...
      <div className="mt-3"><ListRowsSkeleton rows={7} compact /></div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-1">
        {Array.from({ length: 7 }, (_, index) => <SkeletonBlock key={index} className="h-10 w-14" />)}
      </div>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
              {Array.from({ length: 4 }, (_, cardIndex) => (
                <SkeletonCard key={cardIndex}>
                  <SkeletonBlock className="h-5 w-24" />
                  <div className="mt-3 flex gap-1">{Array.from({ length: 10 }, (_, dotIndex) => <SkeletonBlock key={dotIndex} className="h-5 w-5 rounded-full" />)}</div>
                </SkeletonCard>
              ))}
            </div>
          </section>
...
export function CourseDetailSkeleton() {
  return (
    <div>
      <SkeletonHeading withActions />
      <div className="mt-4"><StatCardsSkeleton count={3} className="sm:grid-cols-3" /></div>
