...
export function CourseList({ courses, showTimetable = false, showConflictCourse = true, year, sem }) {
  const { dataset, getCourses, getMyCourseIds } = useApp()
  const [layout, setLayout] = useState('card')
  const [page, setPage] = useState(1)
  const [conflictCourseData, setConflictCourseData] = useState([])
...
function CourseDisplayHeading({ course }) {
...
function TimetableCourses({ courses, year, sem }) {
  const weekdays = Object.keys(dateEng2zh).filter((date) => courses.some((course) => course.time?.[date]?.length))
  const items = []
  const occupied = new Map()
  for (const course of courses) {
    for (const [date, slots] of Object.entries(course.time || {})) {
      if (!slots.length) continue
      const key = `${date}-${slots.join(',')}`
      if (occupied.has(key)) {
        occupied.get(key).isConflict = true
      } else {
        const item = { ...course, date, slots, isConflict: false }
    ...
    <Card className="overflow-hidden p-0">
      ...
      >
        ...
        {items.map((item) => {
          ...
          const end = timetable[timetable.indexOf(item.slots.at(-1)) + 1] || 'end'
          return (
            <Link
            ...
            >
              <div className="font-bold">{item.isConflict ? '含有多個課程' : item.name.zh}</div>
