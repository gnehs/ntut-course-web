...
import { courseTitle, searchCourseList } from '../lib/courseUtils.js'
import { animateSearchResults } from '../lib/motion.js'
...
export function UniversalSearch({ navbar = false, className = '', motionItem = false }) {
  ...
  function remember(item) {
    ...
    const history = JSON.parse(localStorage.getItem('search-history') || '[]').filter((entry) => entry.to !== item.to)
    localStorage.setItem('search-history', JSON.stringify([...history, historyItem].slice(-10).reverse().filter((x, index, array) => array.findIndex((y) => y.id === x.id) === index).reverse()))
    setInput('')
    setFocused(false)
    setCurrentSelectionIndex(-1)
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-search-results
        className={cn(
          'absolute left-0 right-0 top-full z-[999] mt-2 max-h-[512px] overflow-y-auto rounded-[8px] bg-[rgb(var(--vs-background))] shadow-[0_20px_20px_0_rgba(0,0,0,var(--vs-background-opacity,0.02))] transition-all duration-200',
          focused ? 'pointer-events-auto opacity-100' : 'pointer-events-none -translate-y-2 opacity-0',
          'max-h-[256px] md:max-h-[512px]'
        )}
      >
        {focused && items.map((item, index) => (
          <Link
          ...
          >
            <div className={cn('flex-1', navbar ? 'px-3 py-2' : 'px-4 py-3')}>
              <div className="text-xs opacity-80">{item.category || (item.history ? '歷史' : '')}</div>
            ...
            <div className={cn('transition-transform duration-200', navbar ? 'px-3 py-2' : 'px-4 py-3', currentSelectionIndex === index && 'translate-x-[3px]')}>
              {item.history ? (
                currentSelectionIndex === index ? <i className="bx bx-right-arrow-alt text-2xl" /> : <i className="bx bx-history text-2xl" />
              ) : (
