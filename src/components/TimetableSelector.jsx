...
import { dateEng2zh, timetable as defaultTimetable } from '../lib/courseUtils.js'

const defaultDays = ['mon', 'tue', 'wed', 'thu', 'fri']

export function TimetableSelector({ value, onToggle, slots = defaultTimetable, days = defaultDays, className = '' }) {
  return (
    <div className={cn('overflow-hidden rounded-[12px] border border-[rgba(var(--vs-text),0.1)] bg-[rgba(var(--vs-text),0.03)]', className)}>
      <div className="grid grid-cols-[42px_repeat(5,minmax(0,1fr))]">
        ...
        {slots.map((slot) => (
          <TimetableRow key={slot} dayKeys={days} onToggle={onToggle} selected={value} slot={slot} />
        ))}
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      {dayKeys.map((day) => {
        ...
        return (
          <div key={`${day}-${slot}`} className="border-l border-t border-[rgba(var(--vs-text),0.08)] first:border-l-0">
            <button
              type="button"
              aria-pressed={active}
...
function SelectorButton({ children, className = '', ...props }) {
  return (
    <button
      type="button"
      className={cn(
