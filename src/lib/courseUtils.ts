import type { CalendarCourse, Course, CourseName, CourseTime, NamedCourseItem, WeekdayKey } from '../types/course'
...
export const dateEng2zh: Record<WeekdayKey, string> = { sun: '週日', mon: '週一', tue: '週二', wed: '週三', thu: '週四', fri: '週五', sat: '週六' }
...
export function createIcsEvent({ title, location, start, end, description }: { title: string; location?: string; start: Date; end: Date; description?: string }) {
...
export function buildCourseCalendar({ courses, startDate, untilDate, year, sem }: { courses: CalendarCourse[]; startDate: string; untilDate: string; year: string; sem: string }) {
  const events: string[] = []
