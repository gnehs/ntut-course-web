...
import { createSearchParams, replaceQuery } from './urlState.js'

describe('urlState', () => {
  it('writes plain query strings without JSON quoting values', () => {
    replaceQuery('/advanced-search', { year: '115', sem: '1', d: 'main', q: JSON.stringify({ k: '國文' }) })
    ...
    expect(new URLSearchParams(window.location.search).get('q')).toBe(JSON.stringify({ k: '國文' }))
  })
  ...
  it('serializes object values before writing query strings', () => {
    ...
    expect(JSON.parse(new URLSearchParams(window.location.search).get('q'))).toEqual({ tf: timetableFilter })
  })
