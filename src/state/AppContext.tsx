...
export function AppProvider({ children }: { children: React.ReactNode }) {
	...
	const myCourseKey = useCallback(
		(year = dataset.year, sem = dataset.sem, department = dataset.department) => {
			const suffix = department && department !== 'main' ? `-${department}` : '';
			return `my-couse-data-${year}-${sem}${suffix}`;
		},
		[dataset],
	);
	const myCourseClassKey = useCallback(
		(year = dataset.year, sem = dataset.sem) => `my-couse-class-${year}-${sem}`,
		[dataset],
	);

	const getMyCourseIds = useCallback(
		(year = dataset.year, sem = dataset.sem, department = dataset.department) =>
			readJsonStorage(myCourseKey(year, sem, department), []),
		[dataset, myCourseKey],
	);

	const addCourse = useCallback(
		(id: string, year = dataset.year, sem = dataset.sem, department = dataset.department) => {
	...
	const removeCourse = useCallback(
		(id: string, year = dataset.year, sem = dataset.sem, department = dataset.department) => {
			...
			writeJsonStorage(
				key,
				readJsonStorage(key, []).filter((item) => item !== id),
			);
		},
		[dataset, myCourseKey],
	);
