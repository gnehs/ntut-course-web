...
const { apiUrl } = createApi(
	process.env.BASE_URL || 'https://gnehs.github.io/ntut-course-crawler-node',
);
	...
	...Object.entries(yearSems)
		...
		.map(async ({ year, sem }) => {
			...
			courses.map((course) => {
				...
				fs.writeFileSync(
					`./dist/course/${year}/${sem}/${course.id}.html`,
					dom.serialize().replace(/&amp;/g, '&'),
				);
			...
			departments.map((department) => {
				department.class.map((x) => {
					...
					updateTags({ title, description, image, url });
					fs.writeFileSync(
						`./dist/class/${year}/${sem}/${x.name}.html`,
						dom.serialize().replace(/&amp;/g, '&'),
					);
					sitemapUrls.push(url);
				});
			...
			console.log(
				`Generate routes: ${year}/${sem} for ${departments.map((x) => x.class).flat().length} classes`,
			);
		}),
);
fs.mkdirSync(`./dist/teacher/`, { recursive: true });
tasks.push(
	...withdrawal.data.map((x) => {
		let title = x.name;
